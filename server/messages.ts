...
const validateMessageData = (senderUuid: string, messageText: string, offerId: string) => {
    ...
    return messageSchema.parse({ senderUuid, messageText, offerId });
};
...
const generateEmailBody = (offerId: string, isClient: boolean) => {
    const message = isClient ? "Работодателят ви ви изпрати съобщение през Errandix" : "Кандидатът ви за работа ви изпрати съобщение през Errandix";
    const linkText = isClient ? "работодател" : "кандидат";
    const linkUrl = `https://www.errandix.bg/chat/${offerId}`;

    return `
        <!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
        <html dir="ltr" xmlns="http://www.w3.org/1999/xhtml" xmlns:o="urn:schemas-microsoft-com:office:office" lang="bg">
        <head>
            <meta charset="UTF-8">
        ...
        <body style="width:100%;font-family:arial, 'helvetica neue', helvetica, sans-serif;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;padding:0;Margin:0">
            <div dir="ltr" class="es-wrapper-color" lang="bg" style="background-color:#FAFAFA">
                                                                ...
                                                                <table cellpadding="0" cellspacing="0" width="100%" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px">
                                                                    <tr>
                                                                        <td align="center" style="padding:0;Margin:0;padding-bottom:20px;font-size:0px">
                                                                            <a target="_blank" href="https://www.eventify.bg/" style="-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;text-decoration:underline;color:#666666;font-size:14px">
                                                                                <img src="https://www.errandix.bg/logo.png" alt="Logo" style="display:block;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic" width="200" title="Logo">
                                                                            </a>
                                                                        </td>
                                                                    </tr>
                                                                </table>
                                                                ...
                                                                <table cellpadding="0" cellspacing="0" width="100%" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px">
                                                                    ...
                                                                    <tr>
                                                                        <td align="center" style="padding:0;Margin:0;padding-top:10px;padding-bottom:10px;font-size:0px">
                                                                            <img class="adapt-img" src="https://ecgujso.stripocdn.email/content/guids/CABINET_b297a3f42cdd825f6211aa1ebcaf0723/images/82701618311893051.png" alt style="display:block;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic" width="260">
                                                                        </td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td align="center" class="es-m-p0r es-m-p0l" style="Margin:0;padding-top:5px;padding-bottom:5px;padding-left:40px;padding-right:40px">
                                                                            <p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:21px;color:#333333;font-size:14px">
                                                                                Можете да прочетете чата си с ${linkText} на <a href="${linkUrl}">${linkUrl}</a>
                                                                            </p>
                                                                    ...
                                                                    <tr>
                                                                        <td align="center" style="padding:0;Margin:0;padding-top:20px;padding-bottom:20px"></td>
                                                                    </tr>
                                                                </table>
                                                            </td>
                                                        </tr>
                                                                    ...
                                                                    <tr>
                                                                        <td align="center" style="padding:0;Margin:0;padding-bottom:35px">
                                                                            <p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:18px;color:#333333;font-size:12px">
                                                                                Errandix.bg © 2024 All Rights Reserved.
                                                                            </p>
                                                                            <p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:18px;color:#333333;font-size:12px">
                                                                                26 Positano Str, Sofia, Bulgaria
                                                                            </p>
                                                                        </td>
                                                                    </tr>
                                                                </table>
...
export async function sendMessage(senderUuid: string, messageText: string, offerId: string) {
    try {
        const validatedData = validateMessageData(senderUuid, messageText, offerId);

        const allOffers = await db.select({
            clientUuid: offers.clientUuid,
            errandUuid: offers.errandUuid,
        }).from(offers).where(eq(offers.uuid, validatedData.offerId)).execute();

        const currentOffer = allOffers[0];
        if (!currentOffer) throw new Error('Offer not found');

        const errandDetails = await db.select({
            uploaderUuid: errands.uploaderUuid,
        }).from(errands).where(eq(errands.uuid, currentOffer.errandUuid)).execute();

        const currentErrand = errandDetails[0];
        if (!currentErrand) throw new Error('Errand not found');
        ...
        if (currentOffer.clientUuid === validatedData.senderUuid) {
            recipientUuid = currentErrand.uploaderUuid;
        } else if (currentErrand.uploaderUuid === validatedData.senderUuid) {
            recipientUuid = currentOffer.clientUuid;
            isClient = true;
        } else {
            throw new Error('Invalid sender');
        }

        const recipientInfo = await clerkClient.users.getUser(recipientUuid);
        if (!recipientInfo.primaryEmailAddress) throw new Error('Recipient email not found');

        const emailSubject = isClient ? "Работодателят ви ви изпрати съобщение през Errandix" : "Кандидатът ви за работа ви изпрати съобщение през Errandix";
        const emailBody = generateEmailBody(validatedData.offerId, isClient);

        const info = await transporter.sendMail({
            from: `"Errandix" <${process.env.EMAIL_FROM}>`,
            to: recipientInfo.primaryEmailAddress.emailAddress,
        ...
        await db.insert(messages).values(validatedData).execute();
    } catch (error) {
