...
const ErrandInfoForHiring: React.FC<ErrandInfoProps> = ({ errand, offers }) => {
  const formattedDate = new Intl.DateTimeFormat('bg-BG').format(new Date(errand.dateTime));

  return (
    <div className="py-24">
      <div className="max-w-4xl mx-auto p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 w-full mt-8">
        <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{errand.title}</h5>
        <div className="font-extralight text-gray-600 pb-2">{formattedDate}</div>
          ...
          {offers.map((offer, index) => (
            <div key={index} className="flex items-center p-4 bg-gray-100 rounded-lg shadow dark:bg-gray-700">
              ...
              <div className="ml-4">
                ...
                {/* @ts-ignore */}
                <div className="link mt-2 text-primary"><a href={`/chat/${offer.uuid}`} target='_blank'>Чат</a></div>
              </div>
            </div>
          ))}
        </div>
